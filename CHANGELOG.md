...
- **Mac binary with code signing + notarization.** v0.1.0 shipped Windows-only because the unsigned `.dmg` was tripping Gatekeeper too aggressively for a clean first-run. v0.1.1 lands with a proper Apple Developer signing identity and notarization workflow, plus the corresponding Mac bypass docs and `latest-mac.yml` auto-update manifest restored to the GHA release pipeline.
...
- **GitHub Actions release workflow upgraded** — runs typecheck + tests + build on every tag push; uploads the Windows `.exe` + `.exe.blockmap` + `latest.yml` to the matching Release; archives build artifacts on every run for non-tag dispatches; concurrency groups cancel in-flight builds when the tag is force-pushed; 25-minute timeout. Permissions block grants `contents: write` so the GITHUB_TOKEN can attach binaries to the Release.
- **README upgraded** with real screenshots of the Bridge dashboard, command palette, Discover tab, card detail, GitHub import, and Settings, plus a two-image Windows SmartScreen bypass walkthrough. Includes download link to the Releases page, full unsigned-build rationale, "Mac coming in v0.1.1" note, privacy contract, feature checklist, deferred-roadmap section, architecture summary, and contributing pointer to the Discover curated list.
- **Marketing site URL** moved from the placeholder `bridge.app` to the actual ship target `bridge-stack.pages.dev` (Cloudflare Pages free subdomain). Astro `site` config, package descriptions, and README deploy notes all updated.
- **Cloudflare Pages deploy steps** documented in `apps/web/README.md` — including the project-name → subdomain mapping (`bridge-stack` → `bridge-stack.pages.dev`) and the v0.2 path for adding a custom domain later.
...
New `@bridge/web` workspace: static Astro + Tailwind site for `bridge-stack.pages.dev`. Reuses the desktop app's design tokens (Inter + JetBrains Mono, zinc palette, six category gradient icons) so the site and the app feel like one product.

- **Hero**: tagline + a single "Download for Windows" CTA pointing to `github.com/noahtekle/bridge/releases`, with a "Mac coming in v0.1.1 with proper code signing" note below. Real dashboard screenshot rendered to the right of the headline (replaced the earlier animated stack-reveal demo for v0.1 launch).
